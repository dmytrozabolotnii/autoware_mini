...
from HPNet.backbone import Backbone
from HPNet.map_encoder import MapEncoder
...
class HPNet(pl.LightningModule):
    ...
    #     errors = (torch.norm(traj_propose - target_traj.unsqueeze(2), p=2, dim=-1) * target_mask.unsqueeze(2)).sum(
    #         dim=-1)  # [(n1,...nb),H,K]
    ...
    #                                                     :], best_mode_index]  # [(n1,...nb),H,F,2]
    #
    #     predict_mask = generate_predict_mask(data['agent']['visible_mask'][agent_mask, :self.num_historical_steps],
    ...
    #     self.log('train_reg_loss_traj_propose', reg_loss_traj_propose, prog_bar=True, on_step=True, on_epoch=True,
    #              batch_size=1, sync_dist=True)
    #     self.log('train_reg_loss_traj_refine', reg_loss_traj_refine, prog_bar=True, on_step=True, on_epoch=True,
    #              batch_size=1, sync_dist=True)
    ...
    #     reg_loss_traj_refine = self.reg_loss_traj(traj_ref[targ_mask], targ_traj[targ_mask])
    #     loss = reg_loss_traj_propose + reg_loss_traj_refine
    #     self.log('val_reg_loss_traj_propose', reg_loss_traj_propose, prog_bar=True, on_step=False, on_epoch=True,
    #              batch_size=1, sync_dist=True)
    #     self.log('val_reg_loss_traj_refine', reg_loss_traj_refine, prog_bar=True, on_step=False, on_epoch=True,
    #              batch_size=1, sync_dist=True)
    ...
    #     scored_predict_traj = unbatch(traj_output[scored_mask, -1],
    #                                   agent_batch[scored_mask])  # [(n1,K,F,2),...,(nb,K,F,2)]
    ...
    #     self.log('val_minJointADE', self.min_joint_ade, prog_bar=True, on_step=False, on_epoch=True,
    #              batch_size=batch_size, sync_dist=True)
    #     self.log('val_minJointFDE', self.min_joint_fde, prog_bar=True, on_step=False, on_epoch=True,
    #              batch_size=batch_size, sync_dist=True)
    ...
    def test_step(self, data, batch_idx):
        ...
        tep = torch.cat(
            [data['agent']['position'][agent_mask, -1:].unsqueeze(1).repeat_interleave(self.num_modes, 1), traj_output],
            dim=-2)  # [(n1+...+nb),K,F+1,2]
        ...
        for i in range(batch_size):
            ...
            for j in range(num_agent_pre_batch[i]):
                for k in range(self.num_future_steps):
                    row = [case_id[i], scored_agent_id[i][j], scored_frame_id[k], scored_timestamp_ms[k],
                           scored_agent_interset[i][j],
                           scored_predict_traj[i][j, 0, k, 0], scored_predict_traj[i][j, 0, k, 1],
                           scored_predict_yaw[i][j, 0, k],
                           scored_predict_traj[i][j, 1, k, 0], scored_predict_traj[i][j, 1, k, 1],
                           scored_predict_yaw[i][j, 1, k],
                           scored_predict_traj[i][j, 2, k, 0], scored_predict_traj[i][j, 2, k, 1],
                           scored_predict_yaw[i][j, 2, k],
                           scored_predict_traj[i][j, 3, k, 0], scored_predict_traj[i][j, 3, k, 1],
                           scored_predict_yaw[i][j, 3, k],
                           scored_predict_traj[i][j, 4, k, 0], scored_predict_traj[i][j, 4, k, 1],
                           scored_predict_yaw[i][j, 4, k],
                           scored_predict_traj[i][j, 5, k, 0], scored_predict_traj[i][j, 5, k, 1],
                           scored_predict_yaw[i][j, 5, k]]
                    rows.append(row)
