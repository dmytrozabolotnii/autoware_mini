...
class PedestrianCrosswalkChecker:
    ...
    def local_path_callback(self, msg):
        ...
        if len(msg.waypoints) > 0 and len(self.crosswalks) > 0 and len(objects) > 0:
            ...
            for crosswalk in crosswalks_on_global_path:
                if crosswalk['polygon'].intersects(local_path.linestring):
                    ...
                    for crosswalk in crosswalks_on_local_path[:]:
                        ...
                        if crosswalk['polygon'].intersects(object_polygon):
                            ...
                            if not self.ignore_static_obstacles or object_path_approach_angle < self.crossing_angle_max_limit or \
                                ...
                                collision_points.add_intersection_points(crosswalk['intersection_points'], z=obj.position.z, vx=0, vy=0, vz=0, distance_to_stop=self.braking_safety_distance_crosswalk, deceleration_limit=0.0, category=CollisionPoints.OBJECT_ON_CROSSWALK)
                        ...
                        # NON-INTERSECTING OBJECTS - CONSIDER TRAJECTORIES
                            ...
                            for path in obj.candidate_trajectories.paths:
                                ...
                                if crosswalk['polygon'].intersects(trajectory_to_check):
                                    ...
                                    distances = np.array(trajectory.linestring.project(shapely.points(intersection_points)))
                                    min_index = np.argmin(distances)
                                    closest_distance_to_object = distances[min_index]
                                    closest_intersection_point = shapely.Point(intersection_points[min_index])
                                    ...
                                    if closest_intersection_path_approach_angle < self.crossing_angle_max_limit or \
                                        ...
                                        collision_points.add_intersection_points(crosswalk['intersection_points'], z=obj.position.z, vx=0, vy=0, vz=0, distance_to_stop=self.braking_safety_distance_crosswalk, deceleration_limit=self.crosswalk_maximum_deceleration, category=CollisionPoints.TRAJECTORY_ON_CROSSWALK)
